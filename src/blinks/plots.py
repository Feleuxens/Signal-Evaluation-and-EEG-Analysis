...
from blinks.blinks import epochs_have_blinks, process_subject_with_blinkdetection, detect_blinks_on_raw
...
def plot_average_data(bids_root: str, epochs_with_blinks: dict[int, Epochs], epochs_without_blinks: dict[int, Epochs], with_asr: bool):
...
def plot_eeg_plus_eog_one_subject(bids_root: str, subject_id: str, config: PipelineConfig):
    """
...
def all_subjects_plotting(bids_root: str, config: PipelineConfig, output_folder: str, with_asr: bool):
    epochs_with_blinks, epochs_without_blinks = load_all_epochs(bids_root, output_folder, with_asr)

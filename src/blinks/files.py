...
def get_filename(output_folder: str, subject_id:str, with_asr: bool, with_blinks: bool) -> str:
    """
...
def save_blink_epochs(
    ...
    file_with_blinks = get_filename(output_folder, subject_id, with_asr, with_blinks=True)
    file_without_blinks = get_filename(output_folder, subject_id, with_asr, with_blinks=False)
...
def read_blink_epochs(
    ...
    file_without_blinks = get_filename(data_folder, subject_id, with_asr, with_blinks=False)
    ...
    if isfile(file_without_blinks):
        ...
        raise FileNotFoundError(f"{data_folder} doesn't have file: {file_without_blinks}")
    
    return epochs_with_blinks, epochs_without_blinks # pyright: ignore[reportReturnType]

def load_all_epochs(bids_root: str, output_folder: str, with_asr: bool) -> tuple[
    dict[int, Epochs],
    dict[int, Epochs]
]:
    with_blinks = {}
    ...
    for i, subject_id in enumerate(subject_ids):
        ...
        epochs_with_blinks, epochs_without_blinks = read_blink_epochs(output_folder, subject_id, with_asr)

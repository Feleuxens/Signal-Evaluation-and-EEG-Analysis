...
def analyze_subject(subject_id, bids_root = "../data/"):
    ...
    bids_path = BIDSPath(subject=subject_id, root=bids_root, datatype='eeg', suffix='eeg', task='jacobsen')
    ...
    bad_channels_var = [channel for channel, z in zip(channel_names, z_var) if np.abs(z) > bad_channel_z_thresh]
    ...
    bad_channels_corr = [channel for channel, z in zip(channel_names, z_corr) if z < -bad_channel_z_thresh]

    automatic_bad_channels = sorted(set(bad_channels_var + bad_channels_corr))

    raw.info['bads'].extend(automatic_bad_channels)

    print(f"Automatically detected bad channels for subject {subject_id}: {raw.info['bads']}")
    ...
    if id_random is None or id_regular is None:
        raise ValueError(f"Descriptions not found. Available: {list(event_dict.keys())}")

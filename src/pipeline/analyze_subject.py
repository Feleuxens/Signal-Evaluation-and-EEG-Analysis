...
from utils.plots import power_spectral_density_plot, ica_topography_plot, one_channel_erp_plot, butterfly_plot, \
    all_channel_erp_plot, plot_channel, plot_topomap
...
def plot_specific_subject(config: PipelineConfig, output_folder, epochs, raw, ica, pipeline_stats):
    if epochs is not None:
        one_channel_erp_plot(raw, epochs, config.epoching.baseline)
        all_channel_erp_plot(epochs, config.epoching.baseline)
        butterfly_plot(epochs)

    if ica is not None:
        ica_topography_plot(ica, raw)
    power_spectral_density_plot(raw, 0, 64)


def plot_average_data(config: PipelineConfig, output_folder):
...
def save_data(output_folder, subject_id, epochs, raw, ica, pipeline_stats):
